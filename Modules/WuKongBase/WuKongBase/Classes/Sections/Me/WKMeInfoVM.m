...
-(AnyPromise*) updateInfo:(NSString*)field value:(NSString*)value {
    ...
    if (![self isValidChinese:value]) {
        return [AnyPromise promiseWithResolverBlock:^(PMKResolver  _Nonnull reject) {
                        NSError *error = [NSError errorWithDomain:LLang(@"名字只能输入中文!")
                                                              code:1001
                                                          userInfo:@{NSLocalizedDescriptionKey: @"名字只能输入中文!"}];
                        reject(error);
                    }];
        }
...
// 判断字符串是否只包含中文字符
- (BOOL)isValidChinese:(NSString *)value {
    // 正则表达式匹配中文字符
    NSString *pattern = @"^[\\u4e00-\\u9fa5]+$"; // 只允许中文字符
