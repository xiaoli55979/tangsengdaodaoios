...
//    } category:WKPOINT_CATEGORY_COMMONSETTING sort:79000];
    ...
    // 聊天密码
    [[WKApp shared] setMethod:@"commonsetting.chatpwd" handler:^id _Nullable(id  _Nonnull param) {
    ...
    } category:WKPOINT_CATEGORY_COMMONSETTING sort:69900];
    
    // 锁屏密码
    [[WKApp shared] setMethod:@"commonsetting.screenpwd" handler:^id _Nullable(id  _Nonnull param) {
            ...
            @"items":@[
                    @{
                        ...
                        @"onClick":^{
                            ...
                            void (^pushPasswordSetVC)(void) = ^{
                                WKScreenPasswordSetVC *vc = [WKScreenPasswordSetVC new];
                                [[WKNavigationManager shared] pushViewController:vc animated:YES];
                            };

                            if (lockScreenPwd) {
                                WKScreenPasswordVC *vcPwd = [WKScreenPasswordVC new];
                                vcPwd.allowBack = YES;
                                vcPwd.onFinished = ^(NSString * _Nonnull pwd) {
                                    [[WKNavigationManager shared] popViewControllerAnimated:NO];
                                    pushPasswordSetVC();
                                };
                                [[WKNavigationManager shared] pushViewController:vcPwd animated:YES];
                            } else {
                                pushPasswordSetVC();  // 直接调用推送方法
                            }
...
//            ],
//        };
//    } category:WKPOINT_CATEGORY_COMMONSETTING sort:69800];
