...
- (void)viewDidLoad {
    ...
    self.avatarImgView.url = [WKAvatarUtil getGroupAvatar:self.channel.channelId];
}
...
didCropToImage:(nonnull UIImage *)image withRect:(CGRect)cropRect
    ...
    [[WKAPIClient sharedClient] fileUpload:path data:data progress:^(NSProgress * _Nonnull progress) {
        ...
        if(error) {
            ...
            [[NSNotificationCenter defaultCenter] postNotificationName:WKNOTIFY_CHANNEL_AVATAR_UPDATE object:weakSelf.channel];
        }
