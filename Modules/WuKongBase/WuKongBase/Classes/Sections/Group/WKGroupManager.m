...
- (void)groupNo:(NSString *)groupNo toUid:(NSString *)toUid complete:(void (^)(NSError * _Nonnull))complete {
    if(_delegate && [_delegate respondsToSelector:@selector(groupManager:groupNo:toUid:complete:)]) {
        [_delegate groupManager:self groupNo:groupNo toUid:toUid complete:complete];
    }
}
