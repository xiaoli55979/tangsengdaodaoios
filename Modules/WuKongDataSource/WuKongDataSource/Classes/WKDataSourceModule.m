...
-(void) setReceiptManager {
    [[WKSDK shared].receiptManager setMessageReadedProvider:^(WKChannel * _Nonnull channel, NSArray<WKMessage *> * _Nonnull messages, WKMessageReadedCallback  _Nonnull callback) {
        ...
        // 构建请求参数，使用 NSNumber 包装 uint64_t 类型
        NSDictionary *parameters = @{
            @"channel_id": channel.channelId,
            @"channel_type": @(channel.channelType),  // 频道类型，使用 NSNumber 包装
            @"message_ids": messageIds  // 消息 ID 数组
        };
        
        // 发起网络请求
        [[WKAPIClient sharedClient] POST:@"message/readed" parameters:parameters].then(^(id response) {
            // 成功的回调
            callback(nil);
        }).catch(^(NSError *error) {
            // 失败的回调
